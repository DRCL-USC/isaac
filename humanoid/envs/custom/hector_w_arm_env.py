...
class HectorFullFreeEnv(LeggedRobot):
    '''
    ...
    #     self.reset_buf |= torch.any(torch.abs(self.base_euler_xyz[:, 0]) > 0.5, dim=-1)  
    #     self.reset_buf |= torch.any(torch.abs(self.base_euler_xyz[:, 1]) > 0.5, dim=-1)  
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[58: 61] = noise_scales.ang_vel * self.obs_scales.ang_vel   # ang vel
        noise_vec[61: 65] = noise_scales.quat * self.obs_scales.quat         # euler x,y
        return noise_vec
...
# ================================================ Rewards ================================================== #
    def _reward_joint_pos(self):
        ...
        # joint_pos = 0.25 * self.actions.clone()
        pos_target = self.ref_dof_pos.clone()
        # print(self.ref_dof_pos)
    ...
    def _reward_default_joint_pos(self):
        ...
        left_arm_yaw = joint_diff[:, 5:7]
        right_yaw_roll = joint_diff[:, 9: 11]
        right_arm_yaw = joint_diff[:,14:16]
        yaw_roll = torch.norm(left_yaw_roll, dim=1) + torch.norm(right_yaw_roll, dim=1) 
        yaw_roll = torch.clamp(yaw_roll - 0.1, 0, 50)
        arm_yaw_roll = torch.norm(left_arm_yaw, dim=1) + torch.norm(right_arm_yaw, dim=1)
        arm_yaw_roll = torch.clamp(arm_yaw_roll -0.1, 0, 25)
        return torch.exp(-yaw_roll * 100) + torch.exp(-arm_yaw_roll * 2) - 0.01 * torch.norm(joint_diff, dim=1)
