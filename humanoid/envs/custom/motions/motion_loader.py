...
class MotionLoader:
        ...
        *,
        ...
        Args:
            ...
            start: Indexes to fetch the first value. If both, ``start`` and ``end` are specified,
                the first and second values will be fetched from the argument ``a`` (dimension 0).
        ...
        *,
        ...
        Args:
            ...
            start: Indexes to fetch the first quaternion. If both, ``start`` and ``end` are specified,
                the first and second quaternions will be fetched from the argument ``q0`` (dimension 0).
    ...
    def _compute_frame_blend(self, times: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Returns:
            (index_0, index_1, blend):
            - index_0: The indexes of the first frame.
            - index_1: The indexes of the second frame.
            - blend: The interpolation coefficient.
        """
    ...
    def sample(
        ...
        Args:
            ...
            times: Motion time used for sampling.
                If not defined, motion data will be randomly sampled uniformly in time.
            duration: Maximum motion duration to sample.
                If not defined, samples will be within the range of the motion duration.
                If ``times`` is defined, this parameter is ignored.

        Returns:
            A tuple of 6 Tensors:
            1) dof_positions (N, num_dofs)
            2) dof_velocities (N, num_dofs)
            3) body_positions (N, num_bodies, 3)
            4) body_rotations (N, num_bodies, 4) [wxyz]
            5) body_linear_velocities (N, num_bodies, 3)
            6) body_angular_velocities (N, num_bodies, 3)
        """
    ...
    def get_dof_index(self, dof_names: List[str]) -> List[int]:
        ...
        Raises:
            AssertionError: If the specified DOF name doesn't exist.
...
if __name__ == "__main__":
    ...
    motion = MotionLoader(args.file, torch.device("cpu"))
