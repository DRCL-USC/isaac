...
class M1FreeEnv(LeggedRobot):
    ...
    def _get_gait_phase(self):
        ...
        # if torch.norm(self.command_input) < 0.2:
        #     # print("stand")
        #     stance_mask[:, 0] =1
        #     stance_mask[:, 1] =1
        # if torch.norm(self.commands[:, :3] , dim=1) < 0.1:
            # stance_mask[:, 0] = 1
            # stance_mask[:, 1] = 1
...
# ================================================ Rewards ================================================== #
    ...
    def _reward_default_joint_pos(self):
        ...
        yaw_roll = torch.norm(left_yaw_roll, dim=1) + torch.norm(right_yaw_roll, dim=1) + torch.abs(joint_diff[:,5]) + torch.abs(joint_diff[:,11])
        yaw_roll = torch.clamp(yaw_roll - 0.4, 0, 50)
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime *= torch.logical_not(torch.logical_and(torch.norm(self.base_lin_vel[:, :2], dim=1) > 0.1, torch.abs(self.base_ang_vel[:, 2]) > 0.1)) #no reward when robot is not moving
